...
function setLinkAdImpressions() {
	...
	if(impressionsSet){
		...
		for (var i=0; i<links.length; i++) {
			...
			if(links[i].rel.indexOf('no_impression')==-1&&nextPage!=''&&(thisPage!=nextPage)&&(thisPage+'index.'!=nextPage)&&(thisPage!=nextPage+'index.')){
...
function s_doPlugins(s) {
	...
	if(s_pageType==""||!s.pageType){
		...
		if (s.eVar1) {
			...
			s.eVar29=sunPrePageName;
			s.eVar40=sunPreChannel;
		}
	...
	if (!s.linkType) {
		...
		var downloadUrl=s.downloadLinkHandler("msi,tar,gz,dmg,deb,rpm,run,bz2,zip,jar");
